...
const PICKER_CHROME_IDS = new Set([
  'wti-element-picker-overlay',
  'wti-element-picker-highlight',
  'wti-element-picker-tooltip',
  'wti-element-picker-cancel',
]);
...
function createOverlay(): HTMLDivElement {
  ...
  overlay.id = 'wti-element-picker-overlay';
...
function createHighlight(
  ...
  highlight.id = 'wti-element-picker-highlight';
...
function createTooltip(
  ...
  tooltip.id = 'wti-element-picker-tooltip';
...
function startPicker(resolve: (element: Element | null) => void, style?: PickerStyle): void {
  ...
  function getSelectableElementAtPoint(x: number, y: number): Element | undefined {
    ...
    return elementsAtPoint.find(el => {
      ...
      if (el.closest('#wti-host')) return false;
      return true;
    });
  }
  ...
  function onClick(e: MouseEvent) {
    if (resolved) {
      ...
      if (e.target instanceof Element && e.target.closest('#wti-host')) return;
      e.preventDefault();
    ...
    e.preventDefault();
    e.stopImmediatePropagation();
    if (e.target instanceof Element && e.target.id === 'wti-element-picker-cancel') {
      finish(null);
      return;
    }
  ...
  function blockPagePointerEvent(e: PointerEvent) {
    if (e.target instanceof Element && e.target.id === 'wti-element-picker-cancel') return;
...
function createCancelButton(accent: string): HTMLButtonElement {
  ...
  cancelButton.id = 'wti-element-picker-cancel';
